...
void String::concatenate(char* c){
    String current(this->array);
    String con(c);
    current.reserve(con.size());
    for(int i = 0; i < con.size(); i++) current.append(con.array[i]);
    delete[] this->array;
    this->array = new char[current.length+1];
    for(int i = 0; i < current.length; ++i) this->array[0] = current.array[0];
    this->array[this->length] = '\0';
};
